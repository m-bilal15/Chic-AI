...
def test_tc_chat_sec_003(page):
    """
    TC_CHAT_SEC_003: XSS via image tag in message

    Security test: Verify XSS via img tag is prevented
    Expected: Image tag sanitized, no script execution
    """

    # Test ID constant
    tc_id = "TC_CHAT_SEC_003"

    print("\n" + "=" * 80)
    print(f"SECURITY TEST: TC_CHAT_SEC_003 - XSS via image tag in message")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        page.screenshot(path=f"results/TC_CHAT_SEC_003_chat_ready.png", full_page=True)
        ...
        page.screenshot(path=f"results/TC_CHAT_SEC_003_payload_entered.png", full_page=True)

        # Send
        page.locator('button:has-text("Send")').first.click()
        time.sleep(3)

        page.screenshot(path=f"results/TC_CHAT_SEC_003_payload_sent.png", full_page=True)
        ...
        page.screenshot(path=f"results/TC_CHAT_SEC_003_PASSED.png", full_page=True)
        print("\n[PASS] Security test passed - injection prevented")

    except AssertionError as e:
        page.screenshot(path=f"results/TC_CHAT_SEC_003_FAILED.png", full_page=True)
        print(f"\n[FAIL] SECURITY ISSUE: {e}")
        raise

    except Exception as e:
        page.screenshot(path=f"results/TC_CHAT_SEC_003_ERROR.png", full_page=True)
        print(f"\n[ERROR] {e}")
        raise

    finally:
        print("\n" + "=" * 80)
        print(f"SECURITY TEST TC_CHAT_SEC_003 COMPLETE")
        print("=" * 80 + "\n")
