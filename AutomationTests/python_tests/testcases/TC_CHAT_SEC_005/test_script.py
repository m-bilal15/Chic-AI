...
def test_tc_chat_sec_005(page):
    """
    TC_CHAT_SEC_005: JavaScript URL injection

    Security test: Verify javascript: URLs are prevented
    Expected: JavaScript URL not executed
    """

    # Test ID constant
    tc_id = "TC_CHAT_SEC_005"

    print("\n" + "=" * 80)
    print(f"SECURITY TEST: TC_CHAT_SEC_005 - JavaScript URL injection")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        page.screenshot(path=f"results/TC_CHAT_SEC_005_chat_ready.png", full_page=True)
        print("[PRECONDITION] Chat page ready\n")

        chat_page = ChatPage(page)

        # SECURITY TEST: Attempt injection
        malicious_payload = "Click here: javascript:alert('test')"
        ...
        page.screenshot(path=f"results/TC_CHAT_SEC_005_payload_entered.png", full_page=True)

        # Send
        page.locator('button:has-text("Send")').first.click()
        time.sleep(3)

        page.screenshot(path=f"results/TC_CHAT_SEC_005_payload_sent.png", full_page=True)
        ...
        page.screenshot(path=f"results/TC_CHAT_SEC_005_PASSED.png", full_page=True)
        print("\n[PASS] Security test passed - injection prevented")

    except AssertionError as e:
        page.screenshot(path=f"results/TC_CHAT_SEC_005_FAILED.png", full_page=True)
        print(f"\n[FAIL] SECURITY ISSUE: {e}")
        raise

    except Exception as e:
        page.screenshot(path=f"results/TC_CHAT_SEC_005_ERROR.png", full_page=True)
        print(f"\n[ERROR] {e}")
        raise

    finally:
        print("\n" + "=" * 80)
        print(f"SECURITY TEST TC_CHAT_SEC_005 COMPLETE")
        print("=" * 80 + "\n")
