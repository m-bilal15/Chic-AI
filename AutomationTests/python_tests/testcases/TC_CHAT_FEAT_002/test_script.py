...
def test_tc_chat_feat_002(page):
    """
    TC_CHAT_FEAT_002: Upload outfit image

    Action: Click 'Upload Outfit', select image
    Expected: Image uploaded and displayed in chat
    """

    # Test ID constant
    tc_id = "TC_CHAT_FEAT_002"

    print("\n" + "=" * 80)
    print(f"TEST: TC_CHAT_FEAT_002 - Upload outfit image")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        page.screenshot(path=f"results/TC_CHAT_FEAT_002_chat_ready.png", full_page=True)
        print("[PRECONDITION] Chat page ready\n")

        # TEST: Feature functionality
        print(f"[TEST] Testing: Click 'Upload Outfit', select image")
        print("-" * 80 + "\n")
