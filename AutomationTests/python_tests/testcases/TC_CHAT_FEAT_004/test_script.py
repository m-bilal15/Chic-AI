...
def test_tc_chat_feat_004(page):
    """
    TC_CHAT_FEAT_004: Navigate to Shop from sidebar

    Action: Click 'Shop' in sidebar
    Expected: Navigates to shop page
    """

    # Test ID constant
    tc_id = "TC_CHAT_FEAT_004"

    print("\n" + "=" * 80)
    print(f"TEST: TC_CHAT_FEAT_004 - Navigate to Shop from sidebar")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        page.screenshot(path=f"results/TC_CHAT_FEAT_004_chat_ready.png", full_page=True)
        print("[PRECONDITION] Chat page ready\n")

        # TEST: Feature functionality
        print(f"[TEST] Testing: Click 'Shop' in sidebar")
        print("-" * 80 + "\n")

        # Navigate to Shop
        print(f"[STEP] Clicking '{menu_item}' in sidebar...")

        menu_link = page.locator('a:has-text("Shop"), button:has-text("Shop")')
        assert menu_link.count() > 0, "Shop menu not found"
        ...
        print("\n[PASS] Navigation to Shop successful")
