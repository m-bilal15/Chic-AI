...
def test_tc_chat_pers_008(page):
    """
    TC_CHAT_PERS_008: Verify consistency in personalized responses

    Tests that AI uses onboarding data for personalized responses
    Expected: Both responses align with same profile data
    """

    # Test ID constant
    tc_id = "TC_CHAT_PERS_008"

    print("\n" + "=" * 80)
    print(f"TEST: TC_CHAT_PERS_008 - Verify consistency in personalized responses")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        page.screenshot(path=f"results/TC_CHAT_PERS_008_chat_ready.png", full_page=True)
        ...
        page.screenshot(path=f"results/TC_CHAT_PERS_008_ai_personalized_response.png", full_page=True)
        ...
        page.screenshot(path=f"results/TC_CHAT_PERS_008_PASSED.png", full_page=True)
        print("\n[PASS] Personalized response received")

    except AssertionError as e:
        page.screenshot(path=f"results/TC_CHAT_PERS_008_FAILED.png", full_page=True)
        print(f"\n[FAIL] {e}")
        raise

    except Exception as e:
        page.screenshot(path=f"results/TC_CHAT_PERS_008_ERROR.png", full_page=True)
        print(f"\n[ERROR] {e}")
        raise

    finally:
        print("\n" + "=" * 80)
        print(f"TEST TC_CHAT_PERS_008 COMPLETE")
        print("=" * 80 + "\n")
