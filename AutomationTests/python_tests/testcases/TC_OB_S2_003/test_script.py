"""
Test Case: TC_OB_S2_003
Title: Verify multi-select functionality (select multiple areas)
Description: Verify multi-select functionality (select multiple areas)
Priority: Critical
Type: Positive
...
def test_tc_ob_s2_003(page, onboarding_page, base_url):
    """
    Test Case: TC_OB_S2_003
    Description: Verify multi-select functionality (select multiple areas)
    Priority: Critical
    ...
    Expected Results:
    1. All three cards are selected simultaneously (highlighted)
    2. Each shows visual selection indicator
    3. Multiple selections are allowed
    """

    print("\n" + "="*70)
    print("TEST: TC_OB_S2_003 - Verify multi-select functionality (select multiple areas)")
    print("="*70)
    ...
    try:
        ...
        page.screenshot(path="results/TC_OB_S2_003_initial.png", full_page=True)
        ...
        page.screenshot(path="results/TC_OB_S2_003_evidence.png", full_page=True)
        

        # Final screenshot
        page.screenshot(path="results/TC_OB_S2_003_final.png", full_page=True)
        print("\n[SCREENSHOT] Final screenshot saved")

        print("\n" + "="*70)
        print("[PASS] TEST PASSED: TC_OB_S2_003")
        print("="*70)
        ...
        page.screenshot(path="results/TC_OB_S2_003_FAILED.png", full_page=True)
        print(f"\n[FAIL] TEST FAILED: {e}")
